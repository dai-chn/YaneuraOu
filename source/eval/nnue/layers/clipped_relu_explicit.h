...
// WeightScaleBits: 前段 affine の重みスケール (既定 6 = QB64)。SFNN fc_0 は 7 (QB128) を使えるようにした (report/51 §7.7)
template <IndexType InputDimensions, int WeightScaleBits = kWeightScaleBits>
class ClippedReLUExplicit {
 public:
  using InputType = std::int32_t;
  using OutputType = std::uint8_t;
  static constexpr int kShift = WeightScaleBits;
  ...
  void Propagate(const InputType* input, OutputType* output) const {
    ...
    if constexpr (kInputDimensions % 32 == 0)
    {
      ...
      for (IndexType i = 0; i < kNumChunks; ++i) {
        const __m256i words0 = _mm256_srai_epi16(_mm256_packs_epi32(
              ...
              _mm256_loadA_si256(&in[i * 4 + 1])), kShift);
        const __m256i words1 = _mm256_srai_epi16(_mm256_packs_epi32(
              _mm256_loadA_si256(&in[i * 4 + 2]),
              _mm256_loadA_si256(&in[i * 4 + 3])), kShift);
      ...
      const auto out = reinterpret_cast<__m128i*>(output);
        for (IndexType i = 0; i < kNumChunks; ++i) {
          const __m128i words0 = _mm_srai_epi16(_mm_packs_epi32(
              ...
              _mm_load_si128(&in[i * 4 + 1])), kShift);
          const __m128i words1 = _mm_srai_epi16(_mm_packs_epi32(
              _mm_load_si128(&in[i * 4 + 2]),
              _mm_load_si128(&in[i * 4 + 3])), kShift);
    ...
    for (IndexType i = 0; i < kInputDimensions; ++i) {
      output[i] = static_cast<OutputType>(
          std::max(0, std::min(127, input[i] >> kShift)));
    }
  }
